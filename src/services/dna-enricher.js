...
// Garante que auto-enrich.js não precisa mudar.
// ─────────────────────────────────────────────────────────────
function canonParaLegado(can) {
  ...
  const mk = (valor, confiancaKey, fonte) => ({
    valor, fonte, confianca: CONF_MAP[confiancaKey] || 'baixa', motivo: null
  });
...
async function enriquecerDnaViaWeb({ sku, fabricante, nome, nivel_busca }) {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    return { ok: false, erro: 'ANTHROPIC_API_KEY nao configurada', campos: camposVazios(), pendente_confirmacao: true };
  }
  ...
  if (trechos.length === 0) {
    return {
      ok: true, encontrado: false, campos: camposVazios(),
...
${JSON.stringify(NTC_OUTPUT_SCHEMA, null, 2)}`;
    ...
    const userContent =
      ...
      `PRODUTO: ${termoBase}\n\n` +
      `Resultados de busca (${trechos.length} fontes — coloque as URLs exatas em fontes[]):\n` +
      trechos.map((t, i) =>
        `[${i + 1}] Título: ${t.titulo}\nURL: ${t.fonte}\nTrecho: ${t.trecho}`
      ).join('\n\n');
    ...
    try {
      ...
      console.error('[DNA] parse JSON IA:', e.message, '| raw:', rawText.substring(0, 200));
      canonico = {};
    }

    // Garante codigo_entrada
    ...
    if (canonico.dna?.ean?.valor != null) {
      ...
      if (!validarGTIN(s)) {
        canonico.dna.ean = { valor: null, confianca: 'nulo', fontes: [], _rebaixado: 'GTIN inválido (checksum)' };
      } else {
        canonico.dna.ean.valor = s;
      }
    }

    // NCM — valida formato + consulta TIPI
    if (canonico.co?.ncm?.valor != null) {
      const limpo = validarNCM(canonico.co.ncm.valor);
      if (!limpo) {
        canonico.co.ncm = { valor: null, confianca: 'nulo', fontes: [], _rebaixado: 'NCM inválido' };
      } else {
        canonico.co.ncm.valor = limpo;
        try {
          const desc = await consultarNCMOficial(limpo);
          if (desc) {
            canonico.co.ncm.confianca = 'confirmado';
            canonico.co.ncm._ncm_tipi = desc;
          }
        } catch (_) { /* ignora — API TIPI pode estar fora */ }
      }
    }

    // Anos — força inteiro no intervalo válido
...
${JSON.stringify(NTC_OUTPUT_SCHEMA, null, 2)}`;
    ...
    if (Array.isArray(canonico.av?.aplicacoes)) {
      for (const ap of canonico.av.aplicacoes) {
        for (const campo of ['ano_inicial', 'ano_final']) {
          if (ap[campo] != null && ap[campo] !== 'atual') {
            const n = parseInt(ap[campo], 10);
            ap[campo] = (!isNaN(n) && n >= 1950 && n <= 2035) ? n : null;
          }
        }
      }
    }

    // ── BRIDGE: canônico → CAMPOS_DNA legado ─────────────────
    // Mantém compatibilidade total com auto-enrich.js e aplicarCamposDna()
    ...
    console.log(`[DNA] Resultado canônico: ${campos_preenchidos}/${CAMPOS_DNA.length} campos | status: ${canonico.status || 'ok'}`);

    return {
      ok: true,
      encontrado,
      campos: camposLegado,        // ← legado: auto-enrich.js usa este sem mudança
      campos_canonico: canonico,   // ← canônico: ntc-normalizer-patch + artifact usam este
